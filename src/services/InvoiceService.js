        ...
        try {
            ...
            if (keyword) {
                query = {
                    $or: [
                        ...
                        { 'customer': mongoose.Types.ObjectId.isValid(keyword) ? new mongoose.Types.ObjectId(keyword) : null }
                    ]
                };
            }
            ...
            if (keyword && !mongoose.Types.ObjectId.isValid(keyword)) {
                ...
                if (customers.length > 0) {
                    ...
                    const invoicesByCustomer = await Invoice.find({ customer: { $in: customerIds } })
                        .populate({
                            path: 'customer',
                            select: 'name email phone address'
                        })
    ...
    async createInvoice(customerId, invoiceDetails) {
        ...
        try {
            ...
            if (customer.role !== "customer") {
                throw new Error("Invalid customer role. Only users with 'customer' role can be added to invoice");
            }
